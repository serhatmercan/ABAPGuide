...
CLASS lcl_main DEFINITION.
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS set_fiter      RETURNING VALUE(rt_filter)     TYPE lvc_t_filt.
  ...
  METHOD show_alv.
    ...
    IF co_container IS INITIAL.
      ...
      IF co_grid IS INITIAL.
        ...
        co_grid->set_table_for_first_display( EXPORTING  i_buffer_active               = space
                                                         is_layout                     = ls_layout
  ...
  METHOD handle_user_command.
    CASE e_ucomm.
      ...
      WHEN 'ADD_LINE'.
        ...
        DATA lv_out           LIKE LINE OF gt_out.
        DATA lv_records_count TYPE c LENGTH 1 = lines( gt_out ).
    ...
    go_grid->refresh_table_display( is_stable = VALUE lvc_s_stbl( row = abap_true
                                                                  col = abap_true ) ).
  ENDMETHOD.
    ...
    LOOP AT rt_fielcat REFERENCE INTO DATA(ls_fieldcat).
      CASE ls_fieldcat->fieldname.
        ...
        WHEN 'TLGHT'.
          ...
          ls_fieldcat->scrtext_l = ls_fieldcat->col_opt.
...
MODULE status_0100 OUTPUT.
  ...
  go_main->show_alv( EXPORTING iv_cont_name = 'CON100'
                               iv_stru_name = 'ZSM_S_STRUCTURE'
                     CHANGING  co_cont      = go_container
                               co_grid      = go_grid
                               ct_data      = gt_out[] ).
ENDMODULE.
" ----------------------------------
...
FORM set_splitter
  ...
  go_splitter = NEW #( parent  = co_container
                       rows    = 2
                       columns = 1 ).

  go_subcontainer1 = go_splitter->get_container( row    = 1
                                                 column = 1 ).

  go_subcontainer2 = go_splitter->get_container( row    = 2
                                                 column = 1 ).

  go_splitter->set_row_height( id     = 1
                               height = 15 ).

  go_document = NEW #( style = 'ALV_GRID' ).

  go_grid = NEW #( i_parent = go_subcontainer2 ).
