...
" ADD WHERE: DATE & TIME
WHERE ( begin_date > @sy-datum OR ( begin_date = @sy-datum AND begin_time >= @sy-uzeit ) )
...
SELECT * FROM zsm_t_data USING CLIENT @lv_client
  INTO TABLE @DATA(lt_data).
...
SELECT lgort FROM mchb
  WHERE matnr = @lv_matnr
    AND werks = '1000'
    AND ( ( clabs > 0 ) OR ( cinsm > 0 ) OR ( cspem > 0 ) )
  INTO CORRESPONDING FIELDS OF TABLE @lt_lgort.

SELECT lgort FROM mspr
  WHERE matnr = @lv_matnr
    AND werks = '1000'
    AND ( ( prlab > 0 ) OR ( prins > 0 ) OR ( prspe > 0 ) )
